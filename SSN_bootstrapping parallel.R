    ...
    for (prefix in prefixes) {
      ...
      bootstrap_model <- function(ssn_obj, n_boot, model_formula, multiple_ws) {
        ...
        bootstrap_iteration <- function(i) {
          ...
          boot_data_folder = file.path(output_folder, "boot_data")
          # Create the output folder if it doesn't exist
          if (!dir.exists(boot_data_folder)) {
            dir.create(boot_data_folder, recursive = TRUE)
          }
          
          write.csv(boot_data_df, file.path(boot_data_folder, paste0("boot_data_", i, ".csv")))
