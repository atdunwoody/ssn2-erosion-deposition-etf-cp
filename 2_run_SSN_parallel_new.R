...
run_ssn_analysis <- function(prefix, type, segment) {
  tryCatch({
    ...
    return(list(prefix = prefix, type = type, segment = segment))
    
  }, error = function(e) {
    message("An error occurred for ", prefix, " - ", type, " - ", segment, ": ", e$message)
    return(NULL)
  })
}
...
with_progress({
  ...
  results <- future_pmap(param_grid, function(prefix, type, segment) {
    p()  # update progress
